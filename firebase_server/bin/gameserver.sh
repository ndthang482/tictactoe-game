...
export JMXARGS="-Djava.rmi.server.hostname=$MYIP -Dcom.sun.management.jmxremote.port=$JMXP -Dcom.sun.management.jmxremote.rmi.port=$JMXP -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.local.only=false"
if [ "${SILENT}" != "YES" ]; then
...
startserver()
{
	...
	${BINARY} -server ${DEBUG} ${GCARGS} ${SYSARGS} ${JDK_EPOLL} ${JMXARGS} -Xmx${MEMORY} -Xms${MEMORY} -classpath ${CLASSPATH} com.game.server.bootstrap.Server -i ${MYHOST} ${CMDLINE} >> "logs/stdout.txt" 2>> "logs/stderr.txt" &
	SERVERPID=$!
